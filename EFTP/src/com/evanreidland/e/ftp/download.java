...
public class download {
	...
	public static void setDownloaderClass(Class<? extends Downloader> downloadClass) {
	...
	private static class DownloadListThread implements Runnable {
		...
		public DownloadListThread(String address, String[] items, String baseOutput) {
			this.items = items;
	...
	public static int downloadList(String address, String file, String baseOutput) {
		try {
			...
			for ( i = 0; i < listItems.length; i++ ) {
				items.add(i < listItems.length - 1 ? listItems[i].substring(0, listItems[i].length() - 1) : listItems[i] );
			}
			String[] listItemsFull = new String[items.size()];
			for ( i = 0; i < items.size(); i++ ) {
				listItemsFull[i] = address + items.get(i);
			}
			new Thread(new DownloadListThread(address, listItemsFull, baseOutput)).start();
	...
	public static void Extract(String address, String output) {
