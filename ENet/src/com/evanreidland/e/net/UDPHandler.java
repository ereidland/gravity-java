	...
	private class ListenThread implements Runnable {
		public void run() {
			try {
				...
				while ( cstate == ConnectionState.CONNECTED || cstate == ConnectionState.LISTENING ) {
					DatagramPacket receivePacket = new DatagramPacket(data, data.length);
					socket.receive(receivePacket);
					
					onReceive(Packet.fromBytes(receivePacket.getData(), receivePacket.getAddress(), receivePacket.getPort()));
				}
			} catch ( Exception e ) {
				e.printStackTrace();
				cstate = ConnectionState.DISCONNECTED;
	...
	public void Send(byte[] data, InetAddress addr, int port, long targetID) {
		try {
			...
			DatagramPacket sendPacket =
					addr == null ? new DatagramPacket(sendData, sendData.length, this.addr, this.port)
