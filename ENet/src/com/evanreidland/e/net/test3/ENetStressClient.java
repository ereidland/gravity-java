...
public class ENetStressClient {
	...
	private static class Client extends UDPHandler {
		...
		public void onReceive(Packet p) {
			...
			if ( timePassed > TEST_TIME ) {
				NetLog.Log("Sent: " + sent + " Received: " + received + " Dropped: " + (sent - received));
				if ( timePassed != 0 ) {
					NetLog.Log("Sent per second: " + (sent/timePassed) + " Dropped: " + ((sent - received)/timePassed));
				}
				Close();
			}
		}
	...
	public static void main(String args[]) throws Exception {
		...
		while ( client.getState() == ConnectionState.CONNECTED ) {
			client.Send(new Bits().writeLong(System.currentTimeMillis()).getBytes());
			client.sent++;
