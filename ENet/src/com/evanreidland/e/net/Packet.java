...
public class Packet
{
	...
	public byte[] toBytes()
	{
		...
		/*
		 * ndata.writeBit(hasID); // Has id. if ( hasID ) {
		 * ndata.writeLong(++ids); }
		 */
	...
	public static Packet fromBytes(byte[] bytes, InetAddress src, int port)
	{
		...
		short size = packetData.readBit() ? packetData.readByte() : packetData
				.readShort();
		if (packetData.readBit())
		{
		...
		/*
		 * packet.hasID = packetData.readBit(); if ( packet.hasID ) {
	...
	public void logInfo()
	{
		NetLog.Log("Packet id/time/size/src: (" + targetID + ", " + timeStamp
				+ " ms, " + data.getEndBits() + " bits, "
				+ (src != null ? src.toString() : "0") + ":" + srcPort + ")");
