...
public class FontResourceManager extends ResourceManager
{
	...
	public Resource load(String address)
	{
		...
		try
		{
			DataInputStream str = new DataInputStream(new FileInputStream(
					engine.getPath() + address + ".efont"));
			byte version = str.readByte();
			if (version != 1)
			{
				engine.Error("Can't load font, '" + address
						+ "' because of bad version (" + version + ").");
			...
			Resource r = engine.loadTexture(address.substring("sprites/"
					.length()) + ".png");
			if (!r.isValid())
			{
				engine.Error("Can't load font, '" + address
						+ "' because of missing texture (.png expected).");
				return Resource.newInvalid();
			}
