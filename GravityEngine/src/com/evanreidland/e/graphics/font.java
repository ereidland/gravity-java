...
public class font
{
	...
	public static boolean buildFont(String fontName, int baseSize,
			boolean bold, boolean antiAlias)
	{
		try
		{
			...
			BufferedImage fontImage = new BufferedImage(newSize, newSize,
			...
			g.setFont(font);
			FontMetrics fontMetrics = fontImage.getGraphics().getFontMetrics(
					font);

			String fname = engine.getPath() + "sprites/font/" + fontName + "x"
					+ baseSize;

			DataOutputStream str = new DataOutputStream(new FileOutputStream(
					fname + ".efont"));
	...
	public static void Render(Resource res, String str, Vector3 pos,
			Vector3 right, Vector3 up, double size, boolean bCenter)
	{
		if (res.getType() == ResourceType.Font)
		{
			graphics.setTexture((Resource) ((FontGraphic) res).getObject());
		}
		graphics.passTriangleList(renderToTriList(res, str, pos, right, up,
				size, bCenter));
	}

	public static TriList renderToTriList(Resource res, String str,
			Vector3 pos, Vector3 right, Vector3 up, double size, boolean bCenter)
	{
		...
		if (res.getType() == ResourceType.Font)
		{
			...
			Vector3 basePos = bCenter ? pos.minus(right.multipliedBy(font
					.getStringWidth(str, size) * 0.5f)) : pos.cloned();
			...
			for (int i = 0; i < str.length(); i++)
			{
				...
				q.applyToProjection(basePos.plus(right.multipliedBy(curLen
						+ chWidth * 0.5f)), right.multipliedBy(size), up
						.multipliedBy(size));
				list.addQuad(q);
				curLen += chWidth;
			}
	...
	public static void Render3d(Resource res, String str, Vector3 pos,
			double size, boolean bCenter)
	{
		Render(res, str, pos, graphics.right, graphics.up, size, bCenter);
	}

	public static void Render2d(Resource res, String str, Vector3 pos,
			double size, boolean bCenter)
	{
		Render(res, str, pos, new Vector3(1, 0, 0), new Vector3(0, 1, 0), size,
				bCenter);
	}
}
