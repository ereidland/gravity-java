...
public class Sprite
{
			...
			double tx1, double tx2, boolean type)
	{
		...
		Vector3 viewAngle = type ? graphics.camera.pos.minus(pos).getAngle()
				: graphics.forward.getAngle();

		Vector3 up = viewAngle.getUp().combined(angle.getUp())
				.multipliedBy(height * 0.5);// .combined(angle.getRight()).multipliedBy(width*0.5);
