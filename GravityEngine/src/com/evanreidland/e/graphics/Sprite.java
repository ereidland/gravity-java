...
public class Sprite
{
	...
	public void renderBillboard(boolean type)
	{
		...
		Vector3 newAngle = type ? graphics.camera.pos.minus(pos).getAngle()
				: graphics.forward.getAngle();
		
		newAngle.add(angle.multipliedBy(1, 0, 1));
		
		Vector3 up = newAngle.getUp().multipliedBy(width * 0.5), right = newAngle
				.getRight().multipliedBy(width * 0.5);
		
		a.pos.setAs(pos.plus(Vector3.Projected(right, up, angle.y + engine.Pi_2
				+ engine.Pi_4)));
		b.pos.setAs(pos.plus(Vector3
				.Projected(right, up, angle.y + engine.Pi_4)));
		c.pos.setAs(pos.plus(Vector3
				.Projected(right, up, angle.y - engine.Pi_4)));
		d.pos.setAs(pos.plus(Vector3.Projected(right, up, angle.y - engine.Pi_2
				- engine.Pi_4)));
	...
	public void renderLongBillboard(Vector3 v1, Vector3 v2, double height,
			double tx1, double tx2, boolean type)
	{
		
		Vector3 screen1 = graphics.toScreen(v1), screen2 = graphics
				.toScreen(v2);
		
		double angle = screen2.minus(screen1).getAngle2d() + engine.Pi_2;
		
		Vector3 up = Vector3.Projected(graphics.right, graphics.up, angle)
				.multipliedBy(height * 0.5);
