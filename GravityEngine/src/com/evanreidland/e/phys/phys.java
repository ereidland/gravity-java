...
public class phys {
	...
	public static Target getTarget(Vector3 launcherPos, Vector3 launcherVel, Vector3 targetPos, Vector3 targetVel, double shotVel) {
		...
		if ( lengthDelta != 0 ) {
			...
			target.pos = targetPos.plus(targetVel.minus(launcherVel).multipliedBy(target.time));
	...
	public static Vector3 getTargetPoint(Vector3 launcherPos, Vector3 launcherVel, Vector3 targetPos, Vector3 targetVel, double shotVel) {
		return getTarget(launcherPos, launcherVel, targetPos, targetVel, shotVel).pos;
	}
}
