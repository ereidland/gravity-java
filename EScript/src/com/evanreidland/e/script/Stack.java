...
public class Stack {
	...
	public Variable at(int index) {
		return (index >= 0 && index < vars.size()) ? vars.get(index) : ((index < 0 && index >= -vars.size()) ? at(index + vars.size()) : new Variable("_null")); 
	}
	
	public Function functionAt(int index) {
		return (index >= 0 && index < funcs.size()) ? funcs.get(index) : ((index < 0 && index >= -funcs.size()) ? functionAt(index + funcs.size()) : new Function.Null()); 
	}
	...
	public Variable addValue(Value v) {
		...
		//System.out.println(var.getName() + ":" + var.toString() + ", " + var.getType().toString());
		vars.add(var);
		return var;
	}
