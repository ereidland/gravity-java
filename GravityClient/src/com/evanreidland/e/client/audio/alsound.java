...
public class alsound
{
	...
	public static Vector3 sourcePos = Vector3.Zero(), sourceVel = Vector3
			.Zero(), listenerPos = Vector3.Zero(),
			listenerVel = Vector3.Zero(), listenerAt = new Vector3(1, 1, 0),
			listenerUp = new Vector3(0, 0, 1);
	private static IntBuffer sources, buffers;
	private static int numFiles = 0;

	private static FloatBuffer fsourcePos, fsourceVel, flistenerPos,
			flistenerVel, flistenerOri;
	...
	public static boolean loadSound(String address, String reg)
	{
		...
		try
		{
			...
			if (data != null)
			{
				AL10.alBufferData(buffers.get(numFiles), data.format,
						data.data, data.samplerate);
	...
	public static boolean init()
	{
		try
		{
			...
			flistenerVel = BufferUtils.createFloatBuffer(3).put(new float[]
			{
					0, 0, 0
			});
			flistenerPos = BufferUtils.createFloatBuffer(3).put(new float[]
			{
					0, 0, 0
			});
			flistenerOri = BufferUtils.createFloatBuffer(6).put(new float[]
			{
					0, 0, -1, 0, 1, 0
			});
